...
export const getVideos = () => fetch(`${API}/api/videos`).then(r => r.json());
export const importUrl = (url) =>
  fetch(`${API}/api/import`, { method:'POST', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ url }) }).then(r => r.json());
export const toggle = (id, enabled) =>
  fetch(`${API}/api/videos/${id}/toggle`, { method:'PATCH', headers:{'Content-Type':'application/json'}, body: JSON.stringify({ enabled }) }).then(r => r.json());
